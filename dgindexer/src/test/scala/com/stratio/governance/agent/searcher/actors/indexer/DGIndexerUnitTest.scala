...
class ExtractorTestParams(s: Semaphore, sourceDao: CustomTestSourceDao, chunk: Array[ElasticObject]) extends DGExtractorParams(sourceDao, sourceDao, 10,10, ExponentialBackOff(10, 10),10,"test") {
...
class CustomTestSourceDao(noAdds: Boolean, value_type: Option[String]) extends ExtractorSourceDao with ReaderElementDao with IndexerSourceDao {
  override def keyValuePairForDataAsset(mdps: List[String]): List[KeyValuePair] = {
    if (!noAdds) {
      ...
      rows.fold[List[KeyValuePair]](List())((a: List[KeyValuePair], b: List[KeyValuePair]) => {
        a ++ b
      }).filter(a => a.getIdentifier != "EmptyDatastore:")
    } else {
      List[KeyValuePair]()
    }
  }
  ...
  override def keyValuePairForBusinessAsset(ids: List[Long]): List[EntityRow] = {
    if (!noAdds) {
      val rows: List[List[KeyValuePair]] = ids.map(id => List(KeyValuePair(id.toString, "OWNER", "{\"name\":\"\",\"value\":\"People\"}", "2018-11-29T10:27:00.000")))
      rows.fold[List[KeyValuePair]](List())((a: List[KeyValuePair], b: List[KeyValuePair]) => {
        a ++ b
      })
    } else {
  ...
  override def keyValuePairForQualityRule(ids: List[Long]): List[EntityRow] = {
    if (!noAdds) {
      val rows: List[List[KeyValuePair]] = ids.map(id => List(KeyValuePair(id.toString, "OWNER", "{\"name\":\"\",\"value\":\"People2\"}", "2018-11-29T10:27:00.000")))
      rows.fold[List[KeyValuePair]](List())((a: List[KeyValuePair], b: List[KeyValuePair]) => {
        a ++ b
      })
    } else {
  ...
  override def businessRulesForQualityRule(ids: List[Long]): List[EntityRow] = {
    if (!noAdds) {
      val rows: List[List[BusinessAsset]] = ids.map(id => List(BusinessAsset(id.toString, "LegalAge", "Legal Age","Published","RULE","2018-09-28T20:45:00.000")))
      rows.fold[List[BusinessAsset]](List())((a: List[BusinessAsset], b: List[BusinessAsset]) => {
        a ++ b
      })
    } else {
...
class DGIndexerUnitTest extends FlatSpec {
  ...
  "Extractor Completed Events Simulation with text typed attribute" should "be processed in Indexer Mock" in {
    ...
    val chunk: Array[ElasticObject] = Array(
      ...
      new ElasticObject("bt/1",Option("financial"),None,Option("Financial Business Term"),"","GLOSSARY","BUSINESS_TERM","stratio",true, new Timestamp(milis), new Timestamp(milis)),
      new ElasticObject("qr/2",Option("dni"),None,Option("DNI Spain"),"","QUALITY","RULES","stratio",true, new Timestamp(milis), new Timestamp(milis))
    )
  ...
  "Extractor DGPB-XXXX Events Simulation" should "be processed in Indexer Mock" in {
    ...
    val milis: Long = 1543424486000l
    val reference: String = "[{\"id\":\"da/2\",\"name\":\"MyDataStore\",\"alias\":\"\",\"description\":\"My DataStore\",\"metadataPath\":\"MyDataStore:\",\"type\":\"SQL\",\"subtype\":\"DS\",\"_tenant\":\"stratio\",\"active\":true,\"discoveredAt\":\"2018-11-28T17:01:26.000\",\"modifiedAt\":\"2018-11-29T10:27:00.000\",\"dataStore\":\"MyDataStore\",\"businessTerms\":[\"FINANTIAL\",\"RGDP\"],\"qualityRules\":[\"qualityRule2\",\"qualityRule1\"],\"keys\":[\"QUALITY\",\"OWNER\"],\"keysValues\":[\"QUALITY:=High\",\"OWNER:=finantial\"]},{\"id\":\"bt/2\",\"name\":\"financial\",\"alias\":\"\",\"description\":\"Financial Business Term\",\"metadataPath\":\"\",\"type\":\"GLOSSARY\",\"subtype\":\"BUSINESS_TERMS\",\"_tenant\":\"\",\"active\":true,\"discoveredAt\":\"2018-11-28T17:01:26.000\",\"modifiedAt\":\"2018-11-29T10:27:00.000\",\"dataStore\":\"GLOSSARY\",\"keys\":[\"OWNER\"],\"keysValues\":[\"OWNER:=People\"]}]"
    val chunk: Array[ElasticObject] = Array(
      new ElasticObject("da/2",Option("MyDataStore"),None,Option("My DataStore"),"MyDataStore:","SQL","DS","stratio",true, new Timestamp(milis), new Timestamp(milis)),
