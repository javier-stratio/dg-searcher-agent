...
import com.stratio.governance.agent.searcher.actors.extractor.MetadataDGExtractor.Chunks
...
object MetadataDGExtractor {
  case class Chunks(l: List[Array[PGNotification]])
}


class MetadataDGExtractor(indexer: ActorRef, params: ExtractorParams, override val circuitBreakerConfig: Config)
