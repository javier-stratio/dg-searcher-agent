...
class PostgresSourceDaoITTest extends FlatSpec {
  ...
  "PostgresDao constructor " should " create all tables if is not " in {
    ...
    val postgresDao: PostgresSourceDao = new PostgresSourceDao(AppConf.sourceConnectionUrl, AppConf.sourceConnectionUser, AppConf.sourceConnectionPassword, database, schema, 1, 4, exponentialBackOff, new AdditionalBusiness("","bt/", "GLOSSARY","BUSINESS_TERM"),true)
